...
export const Flashcard: React.FC<FlashcardProps> = ({ front, back, onNext, onPrev }) => {
  const [isFlipped, setIsFlipped] = useState(false);

  return (
    <Card className="w-full max-w-2xl min-h-[400px] relative">
      <CardContent className="flex flex-col items-center justify-center h-full p-8">
        <div className={`absolute inset-0 flex items-center justify-center p-8 transition-all duration-300 ${isFlipped ? 'opacity-0' : 'opacity-100'}`}>
          <p className="text-2xl text-center">{front}</p>
        </div>
        <div className={`absolute inset-0 flex items-center justify-center p-8 transition-all duration-300 ${isFlipped ? 'opacity-100' : 'opacity-0'}`}>
