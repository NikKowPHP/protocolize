Of course. I have performed the critical review and will now generate the detailed, atomic, and fully explicit to-do list for **Phase H**.

This phase is entirely focused on the backend. It's designed to build the two core automated engines of the application: the **Content Ingestion Pipeline** and the **Reminder Scheduling Engine**. The instructions are explicit and contain all necessary code, prompts, and configuration details for the autonomous agent to implement them successfully.
...
-   [ ] **Task 1.6: Create the Content Ingestion API Route:** Build the main API endpoint that orchestrates the entire pipeline.
    ...
    *   **Action:** Create the file with the following complete content for the `GET` handler.
    ...
    export async function GET(req: NextRequest) {
      ...
      const lastCheckDate = lastEpisode?.publishedAt ? (lastEpisode.publishedAt > sevenDaysAgo ? lastEpisode.publishedAt : sevenDaysAgo) : sevenDaysAgo;
      ...
      for (const video of newVideos) {
        ...
        const existingEpisode = await prisma.episode.findUnique({ where: { sourceUrl } });
        ...
        try {
          ...
          console.error(`Failed to process video ${videoId}:`, error instanceof Error ? error.message : "Unknown error");
        }
      }
...
-   [ ] **Task 1.7: Configure Vercel Cron Job:**
    ...
    *   **Action:** Create or update this file in the project root to define the scheduled job.
    ...
    ```
    *Note: This schedule runs at 8:00 AM UTC every Monday.*
...
-   [ ] **Task 2.3: Add VAPID Keys to Environment:** Add the generated keys and your contact email to the environment variables.
    ...
    *   **Action:** Add the following lines.
    ...
    ```
    *Remember to populate the actual `.env` file with the generated keys.*
      ...
      const notifications = dueReminders.flatMap(reminder => {
        ...
        return reminder.user.pushSubscriptions.map(sub => {
            ...
            return webpush.sendNotification(pushSubscription, payload).catch(error => {
                console.error(`Failed to send notification to ${sub.endpoint.substring(0, 30)}. Error: ${error.message}`);
